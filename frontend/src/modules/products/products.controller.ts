...
export const productsController = {
  ...
  create: (input: Pick<Product, 'name' | 'cost_price' | 'sale_price'> & Partial<Pick<Product, 'tax_percentage' | 'status'>>) =>
    productsService.create(input),
  update: (id: number, input: Partial<Pick<Product, 'name' | 'cost_price' | 'sale_price' | 'tax_percentage' | 'status'>>) =>
